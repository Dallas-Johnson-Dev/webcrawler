...
class UrlQueue(object):
	...
	def add_to_spider(self, url_string):
		if not url_string in self.processed_urls and not url_string in self.spider_urls.get_all():
			self.spider_urls.add(url_string)
	def add_array_to_spider(self, array_of_urls):
		for x in array_of_urls:
			if not x in self.processed_urls and not x in self.spider_urls.get_all():
	...
	def get_next_spider_url(self):
		return self.spider_urls.pop()
	'''
